...
impl P2pManager {
    ...
    pub fn fetch(self: &Arc<Self>, file_id: &str, sender_id: u64) {
        ...
        let _ = self
            .ui_tx
            .send(Event::P2pStatus { file_id: file_id.to_string(), status: "waiting".into() });
        ...
        self.handle.spawn(async move {
            ...
            if inner.pending.remove(&fid).is_some() {
                let _ = mgr.ui_tx.send(Event::P2pFailed {
                    file_id: fid,
    ...
    fn on_cancel(&self, sig: SignalEvent, peer_id: u64) {
        ...
        drop(inner);
        let _ = self.ui_tx.send(Event::P2pFailed {
    ...
    fn transfer_started(&self, file_id: &str) {
        ...
        let _ = self
            .ui_tx
            .send(Event::P2pStatus { file_id: file_id.to_string(), status: "connecting".into() });
    }
    ...
    fn fail_file(&self, file_id: &str, reason: &str) {
        ...
        let _ = self
            .ui_tx
            .send(Event::P2pFailed { file_id: file_id.to_string(), reason: reason.to_string() });
    }
    ...
    async fn run_sender(&self, dc: Arc<dyn webrtc::data_channel::DataChannel>, file_id: String, peer_id: u64, file: ServingFile) {
        while let Some(event) = dc.poll().await {
            match event {
                DataChannelEvent::OnOpen => {
                    eprintln!("[p2p] sender channel open for {file_id}, streaming {} bytes", file.size);
                    let _ = self.ui_tx.send(Event::P2pStatus {
    ...
    async fn run_receiver(&self, dc: Arc<dyn webrtc::data_channel::DataChannel>, file_id: &str, peer_id: u64) {
                    ...
                    if msg.is_string {
                            ...
                            match v.get("type").and_then(|t| t.as_str()) {
                                ...
                                Some("done") => {
                                    if let Some((fid, expected, name, mime, mut bytes, _)) = download.take() {
                                        ...
                                        self.complete(&fid);
                                        let _ = self.ui_tx.send(Event::P2pComplete {
                                            file_id: fid,
                                            mime,
                                            name,
                                            bytes,
                        ...
                        if received - *last_emitted >= 512 * 1024 {
                            *last_emitted = received;
                            let _ = self.ui_tx.send(Event::P2pProgress {
                                file_id: fid.clone(),
                                received,
                                total: *expected,
                            });
...
impl PeerConnectionEventHandler for PcHandler {
    ...
    async fn on_data_channel(&self, data_channel: Arc<dyn webrtc::data_channel::DataChannel>) {
            ...
            let (file_id, bytes) = loop {
                tokio::select! {
                    ...
                    msg = b_ui_rx.recv() => match msg {
                        Some(Event::P2pComplete { file_id, bytes, .. }) => break (file_id, bytes),
                        Some(_) => continue,
